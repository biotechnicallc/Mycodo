...
hostname_file = os.path.join(INSTALL_DIRECTORY, 'mycodo/hostname.config')
if os.path.isfile(hostname_file):
    with open(hostname_file, 'r') as file:
        lines = file.readline().strip()
        if not lines.startswith("https://"):
            first_line = "https://" + lines
    CLOUDFLARE_HOSTNAME = first_line
else:
    CLOUDFLARE_HOSTNAME = 'https://www.biotecnica.us'
